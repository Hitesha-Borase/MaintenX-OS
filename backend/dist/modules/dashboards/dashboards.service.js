...
async function resolveValidTenantAndPlant(tenantId, plantId) {
    let validTenant = "5bce8458-909a-4dd2-b221-614c32ac7c89";
    let validPlant = "83c90534-4761-495c-b2bf-6a61de2260c4";
    try {
        if (tenantId && (0, tenantContext_js_1.isValidUuid)(tenantId)) {
            const checkT = await database_js_1.db.execute((0, drizzle_orm_1.sql) `SELECT id FROM public.tenants WHERE id = ${tenantId} LIMIT 1`);
            const tRows = checkT?.rows || (Array.isArray(checkT) ? checkT : []);
            if (tRows?.[0]?.id) {
    ...
    try {
        if (plantId && (0, tenantContext_js_1.isValidUuid)(plantId)) {
            const checkP = await database_js_1.db.execute((0, drizzle_orm_1.sql) `SELECT id FROM public.plants WHERE id = ${plantId} LIMIT 1`);
            const pRows = checkP?.rows || (Array.isArray(checkP) ? checkP : []);
            if (pRows?.[0]?.id) {
                validPlant = pRows[0].id;
            }
        }
        else {
            const checkP2 = await database_js_1.db.execute((0, drizzle_orm_1.sql) `SELECT id FROM public.plants WHERE tenant_id = ${validTenant} LIMIT 1`);
            const pRows2 = checkP2?.rows || (Array.isArray(checkP2) ? checkP2 : []);
            if (pRows2?.[0]?.id) {
...
class DashboardsService {
    ...
    async getLineLeadDashboard(tenantId) {
        ...
        const dbBatches = await database_js_1.db.select().from(production_js_1.batches).where((0, drizzle_orm_1.eq)(production_js_1.batches.tenantId, validTenant)).orderBy((0, drizzle_orm_1.desc)(production_js_1.batches.createdAt));
    ...
    async logBatchIngredientWeighing(tenantId, payload) {
    ...
    async advanceRecipeStep(tenantId, payload) {
        const { validTenant } = await resolveValidTenantAndPlant(tenantId);
        try {
            await database_js_1.db.execute((0, drizzle_orm_1.sql) `UPDATE public.batch_steps SET status = ${payload.status} WHERE id::text = ${payload.stepId}`);
        }
        catch (e) {
    ...
    async logCcpCheck(tenantId, payload) {
        const { validTenant, validPlant } = await resolveValidTenantAndPlant(tenantId);
        try {
            const dbLines = await database_js_1.db.select().from(masterData_js_1.productionLines).where((0, drizzle_orm_1.eq)(masterData_js_1.productionLines.tenantId, validTenant));
            const lineId = dbLines[0]?.id;
            const dbBatches = await database_js_1.db.select().from(production_js_1.batches).where((0, drizzle_orm_1.eq)(production_js_1.batches.tenantId, validTenant));
            const batchId = dbBatches[0]?.id;
            const dbUsers = await database_js_1.db.select().from(users_js_1.users).where((0, drizzle_orm_1.eq)(users_js_1.users.tenantId, validTenant));
            const operatorId = dbUsers[0]?.id;
            if (lineId && batchId && operatorId) {
                await database_js_1.db.insert(quality_js_1.ccpChecks).values({
    ...
    async saveLineClearance(tenantId, payload) {
        const { validTenant, validPlant } = await resolveValidTenantAndPlant(tenantId);
        try {
    ...
    async saveSealVerification(tenantId, payload) {
        const { validTenant } = await resolveValidTenantAndPlant(tenantId);
        return {
            success: true,
    ...
    async logWipConsumption(tenantId, payload) {
        const { validTenant } = await resolveValidTenantAndPlant(tenantId);
        return {
            success: true,
    ...
    getDynamicStepsForSKU(toSkuName, category = "") {
    ...
    async getChangeoverStatus(tenantId, targetSkuId) {
        const validTenant = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : "aa3183d2-709b-42a8-add1-b2e4b2d873b0";
        try {
            const skusRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `SELECT id, sku_code, name, category FROM skus WHERE tenant_id = ${validTenant}`);
            const ordersRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `SELECT id, order_number, sku_id, status FROM production_orders WHERE tenant_id = ${validTenant}`);
    ...
    async startChangeover(tenantId, payload) {
        ...
        if (payload?.targetSkuId) {
            const [matchedSku] = await database_js_1.db.select().from(masterData_js_1.skus).where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(masterData_js_1.skus.tenantId, validTenant), (0, drizzle_orm_1.eq)(masterData_js_1.skus.id, payload.targetSkuId)));
            if (matchedSku) {
        ...
        try {
            await database_js_1.db
                .update(masterData_js_1.productionLines)
                .set({ status: "CHANGEOVER" })
                .where((0, drizzle_orm_1.eq)(masterData_js_1.productionLines.tenantId, validTenant));
        }
        catch (e) {
    ...
    async getPlantManagerCommandCenter(tenantId, plantId) {
        ...
        try {
         ...
         ORDER BY id ASC;`, [plantId || 'PLT-01']);
            ...
            const processingLogs = hbRes.rows.filter((r) => (r.stage || '').toUpperCase() === 'PROCESSING');
            const packagingLogs = hbRes.rows.filter((r) => (r.stage || '').toUpperCase() !== 'PROCESSING');
            const hasStageLogs = processingLogs.length > 0 && packagingLogs.length > 0;
            const processingTarget = hasStageLogs
                ? processingLogs.reduce((s, r) => s + Number(r.target || 0), 0)
                : Math.round(totalTarget * 0.48);
            const processingActual = hasStageLogs
                ? processingLogs.reduce((s, r) => s + Number(r.actual || 0), 0)
                : Math.round(totalActual * 0.49);
            const packagingTarget = hasStageLogs
                ? packagingLogs.reduce((s, r) => s + Number(r.target || 0), 0)
                : totalTarget - processingTarget;
            const packagingActual = hasStageLogs
                ? packagingLogs.reduce((s, r) => s + Number(r.actual || 0), 0)
            ...
            const teleRowsRes = await client.query(`SELECT id, machine_code as "machineCode", name, line_id as "lineId", stage, status,
         ...
         WHERE plant_id = $1 OR $1 IS NULL
         ORDER BY id ASC;`, [plantId || 'PLT-01']);
            const processingMachines = teleRowsRes.rows.filter((m) => (m.stage || '').toUpperCase() === 'PROCESSING');
            const packagingMachines = teleRowsRes.rows.filter((m) => (m.stage || '').toUpperCase() !== 'PROCESSING');
            // 3. Holding Tanks & WIP Buffers
            const tankRes = await client.query(`SELECT id, resource_id as "resourceId", resource_code as "resourceCode", name,
         ...
         ORDER BY resource_code ASC;`);
            const holdingTanks = tankRes.rows.map((t) => {
         ...
         FROM pm_machine_telemetry WHERE plant_id = $1 OR $1 IS NULL;`, [plantId || 'PLT-01']);
            ...
            const exDetailRes = await client.query(`SELECT id, title, stage, severity, category, asset_or_order as "assetOrOrder",
         ...
         ORDER BY created_at DESC;`, [plantId || 'PLT-01']);
            const processingExceptions = exDetailRes.rows.filter((e) => (e.stage || '').toUpperCase() === 'PROCESSING');
            const packagingExceptions = exDetailRes.rows.filter((e) => (e.stage || '').toUpperCase() !== 'PROCESSING');
            const p1Count = exDetailRes.rows.filter((e) => e.severity === 'P1').length;
    ...
    async getStaffingRoster(tenantId) {
        const validTenant = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : "aa3183d2-709b-42a8-add1-b2e4b2d873b0";
        try {
            const res = await database_js_1.db.execute((0, drizzle_orm_1.sql) `SELECT id, name, designation, shift_code, is_available, certifications FROM staff WHERE tenant_id = ${validTenant} ORDER BY created_at DESC`);
            const rows = res.rows || [];
            return rows.map((r, i) => {
    ...
    async addStaffOperator(tenantId, payload) {
        const validTenant = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : "aa3183d2-709b-42a8-add1-b2e4b2d873b0";
        try {
            ...
            const [plantRow] = (await database_js_1.db.execute((0, drizzle_orm_1.sql) `SELECT id FROM plants WHERE tenant_id = ${validTenant} LIMIT 1`)).rows;
            const defaultPlantId = plantRow?.id || "bead41e2-b735-41b8-bd00-bdba1682fb6a";
    ...
    async deleteStaffOperator(tenantId, id) {
        const validTenant = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : "aa3183d2-709b-42a8-add1-b2e4b2d873b0";
        try {
            await database_js_1.db.execute((0, drizzle_orm_1.sql) `DELETE FROM staff WHERE id = ${id} AND tenant_id = ${validTenant}`);
    ...
    async getProductionPerformance(tenantId) {
        const validTenant = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : "aa3183d2-709b-42a8-add1-b2e4b2d873b0";
        try {
            const ordersRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
        ...
        LIMIT 1
      `);
            const lineRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
        SELECT nominal_speed_bpm FROM production_lines WHERE tenant_id = ${validTenant} LIMIT 1
      `);
            const order = (ordersRes.rows && ordersRes.rows[0]);
            const lineRow = (lineRes.rows && lineRes.rows[0]);
            if (order) {
    ...
    async getRecoveryStatus(tenantId) {
        ...
        try {
            const orderRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
      ...
      `);
        ...
        try {
            const downtimeRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
      ...
      `);
        ...
        let logs = [];
        try {
            const plansRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
        SELECT id, scenario_name, type, projected_recovery_units, status, created_at 
        FROM pm_recovery_plans 
        ORDER BY created_at DESC LIMIT 20
      `);
            countermeasures = (plansRes.rows || []).map((p) => ({
                id: p.id,
            ...
            logs = (plansRes.rows || []).filter((p) => p.status === 'ACTIVE' || p.status === 'AUTHORIZED').map((p) => ({
    ...
    async getEscalations(tenantId) {
        ...
        try {
            const rows = await database_js_1.db
                .select()
                .from(plantManager_js_1.pmExceptions)
                .orderBy((0, drizzle_orm_1.desc)(plantManager_js_1.pmExceptions.createdAt));
    ...
    async dispatchEscalation(tenantId, payload) {
        ...
        try {
            await database_js_1.db.insert(plantManager_js_1.pmExceptions).values({
                id,
                tenantId: validTenant,
                plantId: "PLT-01",
    ...
    async getNotifications(tenantId, role = "LINELEAD") {
        const validTenant = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : "aa3183d2-709b-42a8-add1-b2e4b2d873b0";
        try {
            const rows = await database_js_1.db
                .select()
                .from(common_js_1.notifications)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(common_js_1.notifications.tenantId, validTenant), (0, drizzle_orm_1.or)((0, drizzle_orm_1.eq)(common_js_1.notifications.targetRole, role), (0, drizzle_orm_1.eq)(common_js_1.notifications.targetRole, "ALL"), (0, drizzle_orm_1.eq)(common_js_1.notifications.targetRole, role.toUpperCase()))))
    ...
    async createNotification(tenantId, payload) {
        const validTenant = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : "aa3183d2-709b-42a8-add1-b2e4b2d873b0";
        try {
            const inserted = await database_js_1.db.insert(common_js_1.notifications).values({
                tenantId: validTenant,
                title: payload.title,
    ...
    async markNotificationRead(tenantId, id) {
        const validTenant = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : "aa3183d2-709b-42a8-add1-b2e4b2d873b0";
        try {
            if ((0, tenantContext_js_1.isValidUuid)(String(id))) {
                await database_js_1.db.update(common_js_1.notifications)
                    .set({ isRead: true })
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(common_js_1.notifications.tenantId, validTenant), (0, drizzle_orm_1.eq)(common_js_1.notifications.id, String(id))));
            }
        }
        catch (e) {
    ...
    async deleteNotification(tenantId, id) {
        ...
        try {
            if ((0, tenantContext_js_1.isValidUuid)(String(id))) {
                await database_js_1.db.delete(common_js_1.notifications)
                    .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(common_js_1.notifications.tenantId, validTenant), (0, drizzle_orm_1.eq)(common_js_1.notifications.id, String(id))));
            }
        }
        catch (e) {
    ...
    async markAllNotificationsRead(tenantId, role = "LINELEAD") {
        const validTenant = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : "aa3183d2-709b-42a8-add1-b2e4b2d873b0";
        try {
            await database_js_1.db.update(common_js_1.notifications)
                .set({ isRead: true })
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(common_js_1.notifications.tenantId, validTenant), (0, drizzle_orm_1.or)((0, drizzle_orm_1.eq)(common_js_1.notifications.targetRole, role), (0, drizzle_orm_1.eq)(common_js_1.notifications.targetRole, "ALL"))));
        }
        catch (e) {
    ...
    async clearAllNotifications(tenantId, role = "LINELEAD") {
        const validTenant = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : "aa3183d2-709b-42a8-add1-b2e4b2d873b0";
        try {
            await database_js_1.db.delete(common_js_1.notifications)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(common_js_1.notifications.tenantId, validTenant), (0, drizzle_orm_1.or)((0, drizzle_orm_1.eq)(common_js_1.notifications.targetRole, role), (0, drizzle_orm_1.eq)(common_js_1.notifications.targetRole, "ALL"))));
    ...
    parseStaffCertifications(certData) {
        const list = [];
        if (!certData)
            return list;
        ...
        if (typeof certData === 'object') {
            ...
            if (Array.isArray(certData.skillDetails) && certData.skillDetails.length > 0) {
                certData.skillDetails.forEach((sk) => {
                    list.push({
    ...
    async getUserProfile(tenantId) {
        ...
        try {
            const userRows = await database_js_1.db
                .select()
                .from(users_js_1.users)
                .where((0, drizzle_orm_1.eq)(users_js_1.users.email, "linelead@maintenx.com"))
                .limit(1);
            if (userRows && userRows[0]) {
                ...
                if (u.phone)
                    userObj.phone = u.phone;
            }
            // Query staff table for live skills/certifications
            let staffRows = await database_js_1.db
                .select()
                .from(masterData_js_1.staff)
                .where((0, drizzle_orm_1.sql) `${masterData_js_1.staff.name} ILIKE ${'%' + userObj.name + '%'} OR ${masterData_js_1.staff.employeeCode} = 'EMP-1048' OR ${masterData_js_1.staff.employeeCode} = ${userObj.id}`)
                .limit(1);
            if (!staffRows || staffRows.length === 0) {
                ...
                const [inserted] = await database_js_1.db.insert(masterData_js_1.staff).values({
                    tenantId: validTenant,
    ...
    async updateUserProfile(tenantId, payload) {
        try {
            ...
            await database_js_1.db.update(users_js_1.users)
                .set({
                firstName,
                lastName,
    ...
    async getOperatorDashboard(tenantId) {
      ...
      `);
            ...
            const poRows = Array.isArray(poRes) ? poRes : (poRes?.rows || []);
            ...
            const hbRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
      ...
      `);
            ...
            const ccpRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
      ...
      `);
            ...
            const ccpRows = Array.isArray(ccpRes) ? ccpRes : (ccpRes?.rows || []);
            if (ccpRows.length > 0) {
                ccpRows.forEach((c) => {
    ...
    async getOperatorJobs(tenantId) {
      ...
      `);
            ...
            return rows.map((r) => {
                ...
                if (r.status === 'PAUSED' || r.status === 'Paused')
                    displayStatus = "Paused - Equipment Breakdown";
                return {
                    id: r.id,
    ...
    async getWorkInstructions(tenantId, targetOrderNumber) {
        const isTenantActive = (0, tenantContext_js_1.isValidUuid)(tenantId);
        const validTenant = isTenantActive ? tenantId : null;
        let activeOrderNumber = isTenantActive ? "" : "PO-2026-904";
        let productName = isTenantActive ? "No Assigned Order" : "Standard SKU Product";
        let productCode = isTenantActive ? "N/A" : "SKU-PROD";
        let lineName = isTenantActive ? "No Assigned Line" : "Line 1 (Aseptic Bottling)";
        let workInstructions = isTenantActive ? "No active work instructions for this tenant." : "SOP-PKG-042: High-Speed Aseptic Cold Fill & Nitrogen Flush Procedures v4.1";
        ...
        try {
            ...
            if (targetOrderNumber && targetOrderNumber.trim()) {
                ...
                activeRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
          SELECT 
            po.id, 
            po.order_number as "orderNumber", 
            s.sku_code as "productCode", 
        ...
        `);
            ...
            if ((!rows || rows.length === 0) && validTenant) {
                // Fallback to active/latest order strictly scoped to the same tenant
          ...
          LIMIT 1
        `);
                const fallbackRows = fallbackRes?.rows || (Array.isArray(fallbackRes) ? fallbackRes : []);
            ...
            const rtgRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
        ...
        LIMIT 1
      `);
            const rtgRows = rtgRes?.rows || (Array.isArray(rtgRes) ? rtgRes : []);
            let routingCode = productCode;
            let routingId = null;
            if (rtgRows.length > 0) {
                if (rtgRows[0]?.routing_code)
                    routingCode = rtgRows[0].routing_code;
                if (rtgRows[0]?.id)
                    routingId = rtgRows[0].id;
            }
            ...
            if (routingId) {
                const stepsRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
          SELECT 
            sequence, 
        ...
        `);
                const sRows = stepsRes?.rows || (Array.isArray(stepsRes) ? stepsRes : []);
                if (sRows.length > 0) {
                    stepsList = sRows.map((st, idx) => ({
            ...
            const sigRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
        ...
        LIMIT 1
      `);
            const sigRows = sigRes?.rows || (Array.isArray(sigRes) ? sigRes : []);
            if (sigRows.length > 0) {
                acknowledged = true;
            }
        }
    ...
    async submitProductionLog(tenantId, payload) {
        ...
        try {
            ...
            if (payload.orderId || payload.orderNumber) {
                ...
                const ordRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
          SELECT id, plant_id, line_id, produced_quantity, scrap_quantity 
          FROM public.production_orders 
          WHERE id::text = ${searchVal} OR order_number = ${searchVal}
          LIMIT 1
        `);
                const rows = ordRes?.rows || (Array.isArray(ordRes) ? ordRes : []);
                if (rows.length > 0)
                    targetOrder = rows[0];
            }
            if (!targetOrder) {
          ...
          LIMIT 1
        `);
                const rows = fallbackRes?.rows || (Array.isArray(fallbackRes) ? fallbackRes : []);
                if (rows.length > 0)
                    targetOrder = rows[0];
            }
            if (targetOrder) {
                const tenantIdForLog = targetOrder.tenant_id || validTenant;
                const plantId = targetOrder.plant_id;
                const lineId = targetOrder.line_id;
                // Dynamic lookup valid operator user in this tenant
                let operatorId = null;
                const userRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
          SELECT id FROM public.users 
          WHERE tenant_id = ${tenantIdForLog} AND email LIKE '%operator%'
          LIMIT 1
        `);
                const uRows = userRes?.rows || (Array.isArray(userRes) ? userRes : []);
                if (uRows.length > 0 && uRows[0].id) {
                    operatorId = uRows[0].id;
                }
                else {
                    const fallbackUser = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
            SELECT id FROM public.users WHERE tenant_id = ${tenantIdForLog} LIMIT 1
          `);
                    const fbRows = fallbackUser?.rows || (Array.isArray(fallbackUser) ? fallbackUser : []);
                    if (fbRows.length > 0 && fbRows[0].id)
                        operatorId = fbRows[0].id;
                }
                const hourWindow = `${new Date().getHours()}:00 - ${new Date().getHours() + 1}:00`;
                const shiftCode = 'Shift A (Day)';
                // 1. Insert into public.shift_logs table if valid references exist
                if (operatorId && plantId && lineId) {
                    await database_js_1.db.execute((0, drizzle_orm_1.sql) `
            INSERT INTO public.shift_logs (
              ...
              ${tenantIdForLog}, ${plantId}, ${lineId}, ${targetOrder.id}, ${shiftCode}, ${hourWindow}, ${operatorId}, ${goodUnits}, ${scrapUnits}, NOW()
            )
          `);
                }
    ...
    async logScrapDefect(tenantId, payload) {
        ...
        SELECT id FROM public.users WHERE tenant_id = ${validTenant} AND email LIKE '%operator%' LIMIT 1
      `);
            const uRows = uRes?.rows || (Array.isArray(uRes) ? uRes : []);
            if (uRows.length > 0 && uRows[0].id)
                userId = uRows[0].id;
            const pRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
        SELECT id FROM public.plants WHERE tenant_id = ${validTenant} LIMIT 1
      `);
            const pRows = pRes?.rows || (Array.isArray(pRes) ? pRes : []);
          ...
          VALUES (${validTenant}, ${plantId}, ${userId}, 'SCRAP_DEFECT_REASON', ${payload.defectCode || 'GENERIC_SCRAP'}, 'DEFECT_LOG', ${'Scrap defect logged: ' + (payload.defectCode || '') + ' (' + scrapAdd + ' units) - Notes: ' + (payload.notes || 'N/A')}, NOW())
        `);
            }
        }
        catch (e) {
    ...
    async logOperatorDowntimeEvent(tenantId, payload) {
        ...
        try {
            let realAssetId = "e6807d29-37e2-4d5f-a331-734bc8aae1ba";
            if (payload.assetId && (0, tenantContext_js_1.isValidUuid)(payload.assetId)) {
                realAssetId = payload.assetId;
            }
            else {
                const aRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
          SELECT id FROM public.assets 
          WHERE asset_code = ${payload.assetId} OR id::text = ${payload.assetId} OR name ILIKE ${'%' + (payload.assetId || '') + '%'} 
          LIMIT 1
        `);
                const aRows = aRes?.rows || (Array.isArray(aRes) ? aRes : []);
    ...
    async submitQualityChecklist(tenantId, payload) {
        ...
        try {
            const bRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `SELECT id FROM public.batches LIMIT 1`);
            const bRows = bRes?.rows || (Array.isArray(bRes) ? bRes : []);
    ...
    async submitMaterialRequisition(tenantId, payload) {
        ...
        try {
            const activeRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
        ...
        LIMIT 1
      `);
            const activeRows = activeRes?.rows || (Array.isArray(activeRes) ? activeRes : []);
    ...
    async confirmMaterialReceipt(tenantId, id) {
      ...
      `);
            ...
            if (rows.length > 0 && rows[0].comments) {
                try {
                    parsed = JSON.parse(rows[0].comments);
                }
                catch (e) { }
            }
    ...
    async deleteMaterialRequisition(tenantId, id) {
        const validTenant = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : "aa3183d2-709b-42a8-add1-b2e4b2d873b0";
        try {
            await database_js_1.db.execute((0, drizzle_orm_1.sql) `
    ...
    async parseBarcode(tenantId, payload) {
        ...
        try {
            ...
            const lotRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
        ...
        LIMIT 1
      `);
            const lotRows = lotRes?.rows || (Array.isArray(lotRes) ? lotRes : []);
            if (lotRows.length > 0) {
                const row = lotRows[0];
            ...
            const assetRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
        SELECT id, asset_code, name, status
        FROM public.assets
        WHERE tenant_id = ${validTenant} AND (asset_code ILIKE ${code} OR id::text = ${code} OR name ILIKE ${'%' + code + '%'})
        LIMIT 1
      `);
            const assetRows = assetRes?.rows || (Array.isArray(assetRes) ? assetRes : []);
    ...
    async triggerEmergencyCall(tenantId, payload) {
        ...
        try {
            const uRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `SELECT id FROM public.users LIMIT 1`);
            const uRows = uRes?.rows || (Array.isArray(uRes) ? uRes : []);
        ...
        return [
            {
                ...
                receivedBy: "Douglas Andrew",
                notes: "Smokehouse #3 and Weiler Grinder running nominal. Pre-op inspection passed at 05:30. Multivac R535 seal bar tension verified.",
                status: "SIGNED OFF",
                timestamp: "2026-09-20 05:55"
            }
        ];
    }
    async submitShiftHandoff(tenantId, payload) {
        ...
        try {
            if ((0, tenantContext_js_1.isValidUuid)(tenantId)) {
                ...
                if (rows?.[0]?.id)
                    validTenant = rows[0].id;
            }
            const pRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `SELECT id FROM public.plants WHERE tenant_id = ${validTenant} LIMIT 1`);
    ...
    async getOperatorProfile(tenantId) {
      ...
      `);
        ...
        try {
            const sRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
        SELECT employee_code, name, designation, shift_code, phone, email, certifications 
        FROM public.staff 
        WHERE tenant_id = ${validTenant} 
        LIMIT 1
      `);
            const sRows = sRes?.rows || (Array.isArray(sRes) ? sRes : []);
            if (sRows.length > 0)
                dbStaff = sRows[0];
        }
        catch (e) {
    ...
    async updateOperatorProfile(tenantId, payload) {
        ...
        try {
            ...
            SET first_name = ${firstName},
                last_name = ${lastName},
                email = ${payload.email ? payload.email : (0, drizzle_orm_1.sql) `email`},
                phone = ${payload.phone ? payload.phone : (0, drizzle_orm_1.sql) `phone`},
                updated_at = NOW()
            WHERE tenant_id = ${validTenant}
          `);
                }
                    ...
                    await database_js_1.db.execute((0, drizzle_orm_1.sql) `
            UPDATE public.users 
            SET email = ${payload.email ? payload.email : (0, drizzle_orm_1.sql) `email`},
                phone = ${payload.phone ? payload.phone : (0, drizzle_orm_1.sql) `phone`},
                updated_at = NOW()
            WHERE tenant_id = ${validTenant}
          `);
            ...
            const staffUpdates = [];
            if (payload.name)
                staffUpdates.push((0, drizzle_orm_1.sql) `name = ${payload.name}`);
            if (payload.email)
                staffUpdates.push((0, drizzle_orm_1.sql) `email = ${payload.email}`);
            if (payload.phone)
                staffUpdates.push((0, drizzle_orm_1.sql) `phone = ${payload.phone}`);
            if (payload.shift)
                staffUpdates.push((0, drizzle_orm_1.sql) `shift_code = ${payload.shift}`);
            if (payload.certifications && Array.isArray(payload.certifications)) {
                const jsonStr = JSON.stringify(payload.certifications);
                staffUpdates.push((0, drizzle_orm_1.sql) `certifications = ${jsonStr}::json`);
            }
            staffUpdates.push((0, drizzle_orm_1.sql) `updated_at = NOW()`);
            if (staffUpdates.length > 1) {
                await database_js_1.db.execute((0, drizzle_orm_1.sql) `
          UPDATE public.staff
          SET ${drizzle_orm_1.sql.join(staffUpdates, (0, drizzle_orm_1.sql) `, `)}
          WHERE tenant_id = ${validTenant}
        `);
            }
        }
    ...
    async getSupervisorDashboard(tenantId) {
        try {
            ...
            if (!shiftLead) {
                const [firstUser] = await database_js_1.db
                    ...
                    .limit(1);
                shiftLead = firstUser ? `${firstUser.firstName} ${firstUser.lastName}` : "Ronald Robinson";
            }
            ...
            let processingBatches = [];
            try {
                const pbRes = await database_js_1.pool.query(`
          SELECT 
            b.id,
            b.batch_number as "batchNumber",
            b.recipe_version as "recipeVersion",
        ...
        `);
            ...
            try {
                const staffRows = await database_js_1.db
                    .select()
                    .from(masterData_js_1.staff)
                    .where((0, tenantContext_js_1.isValidUuid)(tenantId) ? (0, drizzle_orm_1.eq)(masterData_js_1.staff.tenantId, tenantId) : (0, drizzle_orm_1.sql) `1=1`);
            ...
            let stageHandoffs = [];
            try {
                const handoffRows = await database_js_1.db
                    .select()
                    .from(plantManager_js_1.pmShiftHandoffs)
                    .where((0, tenantContext_js_1.isValidUuid)(tenantId) ? (0, drizzle_orm_1.eq)(plantManager_js_1.pmShiftHandoffs.tenantId, tenantId) : (0, drizzle_orm_1.sql) `1=1`)
                    .orderBy((0, drizzle_orm_1.desc)(plantManager_js_1.pmShiftHandoffs.createdAt))
            ...
            let stageExceptions = [];
            try {
                const excRows = await database_js_1.db
                    .select()
                    .from(common_js_1.exceptions)
                    .where((0, tenantContext_js_1.isValidUuid)(tenantId) ? (0, drizzle_orm_1.eq)(common_js_1.exceptions.tenantId, tenantId) : (0, drizzle_orm_1.sql) `1=1`)
                    .orderBy((0, drizzle_orm_1.desc)(common_js_1.exceptions.reportedAt))
                    .limit(6);
        ...
        try {
            ...
            return staffList.map(s => {
                ...
                return {
                    ...
                    plant: certs.plant || "Plant 1 - Meat Processing & Smokehouse Facility",
    ...
    async addSupervisorWorkforceEmployee(tenantId, payload) {
        const { validTenant, validPlant } = await resolveValidTenantAndPlant(tenantId, payload?.plantId);
        ...
        const certifications = {
            ...
            plant: payload.plant || "Plant 1 - Meat Processing & Smokehouse Facility",
        ...
        try {
            ...
            staffList.forEach((s, idx) => {
                ...
                if (Array.isArray(certs.skillDetails) && certs.skillDetails.length > 0) {
                    certs.skillDetails.forEach((sk, sIdx) => {
                        skillsList.push({
                            id: `SKL-${idx + 1}-${sIdx + 1}`,
                            staffId: s.id,
    ...
    async addSupervisorSkill(tenantId, payload) {
        ...
        try {
            ...
            const target = await database_js_1.db.query.staff.findFirst({
                where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(masterData_js_1.staff.tenantId, validTenant), (0, drizzle_orm_1.sql) `(${masterData_js_1.staff.name} ILIKE ${'%' + searchEmp + '%'} OR ${masterData_js_1.staff.employeeCode} = ${payload.employeeId || payload.employee})`)
            });
            ...
            if (target) {
                ...
                const updatedDetails = [newSkillDetail, ...existingDetails.filter((d) => d.skillName !== payload.skillName)];
                const updatedCerts = {
                    ...existingCerts,
                    skills: updatedSkills,
    ...
    async updateSupervisorSkillLevel(tenantId, id, payload) {
        ...
        try {
            const searchEmp = payload.employee || payload.employeeId || "";
            const target = await database_js_1.db.query.staff.findFirst({
                where: (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(masterData_js_1.staff.tenantId, validTenant), (0, drizzle_orm_1.sql) `(${masterData_js_1.staff.name} ILIKE ${'%' + searchEmp + '%'} OR ${masterData_js_1.staff.employeeCode} = ${payload.employeeId || payload.employee || ''} OR ${masterData_js_1.staff.id}::text = ${id})`)
            });
            if (target) {
                const existingCerts = target.certifications || {};
                const existingDetails = Array.isArray(existingCerts.skillDetails) ? existingCerts.skillDetails : [];
        ...
        try {
            ...
            staffList.forEach((s, idx) => {
                ...
                if (Array.isArray(certs.trainings) && certs.trainings.length > 0) {
                    certs.trainings.forEach((t, tIdx) => {
                        trainingList.push({
                            id: `TRN-${idx + 1}-${tIdx + 1}`,
                            staffId: s.id,
    ...
    async addSupervisorTraining(tenantId, payload) {
        ...
        try {
            ...
            if (target) {
                ...
                const updatedTrainings = [newTraining, ...existingTrainings.filter((t) => t.program !== payload.trainingProgram)];
                const updatedCerts = {
                    ...existingCerts,
                    trainingStatus: "In Progress",
                    lastTrainingProgram: payload.trainingProgram,
    ...
    async completeSupervisorTraining(tenantId, id, payload) {
        ...
        try {
            ...
            if (target) {
                ...
                const existingTrainings = Array.isArray(existingCerts.trainings) ? existingCerts.trainings : [];
                let updatedTrainings = existingTrainings.map((t) => {
                ...
                if (!updatedTrainings.some((t) => t.program === (payload.trainingProgram || existingCerts.lastTrainingProgram))) {
    ...
    async getSupervisorStaffing(tenantId) {
        try {
            const { validTenant } = await resolveValidTenantAndPlant(tenantId);
            // 1. Fetch real shifts from public.shifts table
            const shiftsRes = await database_js_1.pool.query(`
        SELECT id, tenant_id, plant_id, code, name, start_time, end_time, is_active,
               line, supervisor, planned_headcount, actual_headcount, shift_status, operators
        FROM public.shifts
        WHERE tenant_id = $1
        ORDER BY created_at DESC, name ASC;
      `, [validTenant]);
            const today = new Date().toISOString().substring(0, 10);
            const rosters = shiftsRes.rows.map((sh) => ({
                id: sh.id,
                shiftCode: sh.code,
                shiftName: sh.name,
                shiftTiming: `${sh.start_time || '06:00'} - ${sh.end_time || '14:30'}`,
                date: today,
                line: sh.line || "Line 1 — High-Speed Bottling",
                supervisor: sh.supervisor || "Operations Supervisor",
                operators: Array.isArray(sh.operators) ? sh.operators : [],
                plannedHeadcount: sh.planned_headcount || 1,
                actualHeadcount: sh.actual_headcount || 1,
                shiftStatus: sh.shift_status || (sh.is_active ? "Scheduled" : "Closed")
            }));
            return rosters;
        }
        catch (e) {
    ...
    async addSupervisorStaffing(tenantId, payload) {
        ...
        const line = payload.line || "Line 1 — High-Speed Bottling";
        const supervisor = payload.supervisor || "Operations Supervisor";
        const plannedHeadcount = Number(payload.plannedHeadcount) || 1;
        const actualHeadcount = Number(payload.actualHeadcount) || 1;
        const shiftStatus = payload.shiftStatus || "Scheduled";
        try {
            const insertRes = await database_js_1.pool.query(`
        INSERT INTO public.shifts (
          tenant_id, plant_id, code, name, start_time, end_time, is_active,
          line, supervisor, planned_headcount, actual_headcount, shift_status, operators
      ...
      `, [
                ...
                validPlant,
                code,
                payload.shiftName || "New Production Shift",
                startTime,
                endTime,
                shiftStatus !== "Closed",
                line,
            ...
            return {
                ...
                shiftCode: row.code,
                shiftName: row.name,
                shiftTiming: `${row.start_time} - ${row.end_time}`,
                ...
                message: `Shift "${row.name}" created and saved to PostgreSQL database.`
            };
        }
        catch (e) {
            console.error("Could not insert shift to public.shifts:", e.message);
            throw new Error(`Failed to save shift to database: ${e.message}`);
        }
    }
    async updateSupervisorStaffing(tenantId, id, payload) {
        ...
        SET name = COALESCE($1, name),
            ...
            planned_headcount = COALESCE($6, planned_headcount),
            actual_headcount = COALESCE($7, actual_headcount),
            shift_status = COALESCE($8, shift_status),
      ...
      `, [
                ...
                payload.line,
                payload.supervisor,
                payload.plannedHeadcount ? Number(payload.plannedHeadcount) : null,
                payload.actualHeadcount ? Number(payload.actualHeadcount) : null,
                payload.shiftStatus,
                payload.shiftStatus ? payload.shiftStatus !== "Closed" : null,
                id,
                validTenant
            ]);
        }
    ...
    async assignSupervisorStaffingPersonnel(tenantId, id, payload) {
        ...
        try {
            if (payload.employeeName) {
                await database_js_1.pool.query(`
          UPDATE public.shifts
          SET operators = array_append(operators, $1)
          WHERE id = $2 AND tenant_id = $3 AND NOT ($1 = ANY(operators));
        `, [payload.employeeName, id, validTenant]);
            }
            // Update employee in staff table to assign to this shift if exists
    ...
    async closeSupervisorStaffingShift(tenantId, id, payload) {
        ...
        try {
            await database_js_1.pool.query(`
        UPDATE public.shifts
        SET is_active = false, shift_status = 'Closed'
    ...
    async deleteSupervisorStaffing(tenantId, id) {
        const { validTenant } = await resolveValidTenantAndPlant(tenantId);
        try {
            await database_js_1.pool.query(`DELETE FROM public.shifts WHERE id = $1 AND tenant_id = $2;`, [id, validTenant]);
            return {
                id,
    ...
    async updateSupervisorProductionRun(tenantId, payload) {
        ...
        try {
            ...
            if (payload.orderId || payload.orderNumber) {
                const searchVal = payload.orderId || payload.orderNumber;
                const res = await database_js_1.db.execute((0, drizzle_orm_1.sql) `
          SELECT id, notes FROM public.production_orders 
          WHERE (id::text = ${searchVal} OR order_number = ${searchVal})
          LIMIT 1
        `);
                const rows = res?.rows || (Array.isArray(res) ? res : []);
                if (rows.length > 0) {
                    targetOrderId = rows[0].id;
          ...
          SELECT id, notes FROM public.production_orders 
          WHERE tenant_id = ${validTenant}
          ORDER BY CASE WHEN status = 'RUNNING' OR status = 'Running' THEN 1 ELSE 2 END, updated_at DESC NULLS LAST
          LIMIT 1
        `);
                const rows = res?.rows || (Array.isArray(res) ? res : []);
                if (rows.length > 0) {
          ...
          SET 
            status = ${payload.status},
            produced_quantity = ${String(payload.producedQuantity)},
            scrap_quantity = ${String(payload.scrapQuantity)},
            notes = ${payload.speedBPM ? `Live Running Speed: ${payload.speedBPM} BPM` : existingNotes},
            updated_at = NOW()
    ...
    async getSupervisorProfile(tenantId, userId) {
        try {
            ...
            const userName = userRecord ? `${userRecord.firstName} ${userRecord.lastName}` : "Ronald Robinson";
            const userEmail = userRecord?.email || "supervisor@maintenx.com";
            const userPhone = userRecord?.phone || "+1 (905) 555-0144";
            // 2. Fetch staff record from public.staff table
            let staffRecord = null;
            const matchingStaff = await database_js_1.db
                .select()
                .from(masterData_js_1.staff)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(masterData_js_1.staff.tenantId, validTenant), (0, drizzle_orm_1.eq)(masterData_js_1.staff.name, userName)));
            ...
            const qualifications = rawCerts.qualifications || [
                { name: "HACCP & CFIA Meat Processing Clearance", desc: "Authorized to verify critical control points and thermal lethality.", level: "Level 3", variant: "emerald" },
                { name: "Smokehouse & Thermal Diagnostics", desc: "Master-level meat processing diagnostics and continuous batch monitoring.", level: "Advanced", variant: "emerald" }
            ];
            return {
                id: userRecord?.id || staffRecord?.id,
                name: staffRecord?.name || userName,
                title: staffRecord?.designation || "Processing Operations Supervisor",
                employeeId: staffRecord?.employeeCode || "EMP-1054",
                email: userEmail,
                phone: staffRecord?.phone || userPhone,
                plant: rawCerts.plant || "Plant 1 - Meat Processing & Smokehouse Facility",
                shift: staffRecord?.shiftCode || "Shift A (06:00 - 14:30)",
            ...
            return {
                ...
                title: "Processing Operations Supervisor",
                employeeId: "EMP-1054",
    ...
    async logPackagingOutputCases(tenantId, payload) {
        try {
    ...
    async verifySealAndLabel(tenantId, payload) {
        try {
            return {
                success: true,
