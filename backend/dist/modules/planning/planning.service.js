...
class PlanningService {
    ...
    async resolveSkuId(tenantId, skuIdOrCode) {
        try {
            ...
            const res = await database_js_1.db.execute((0, drizzle_orm_1.sql) `SELECT id, sku_code, name, uom FROM public.skus WHERE tenant_id = ${validTenant}`);
            const skuRows = (res.rows || []);
            if (skuRows.length > 0) {
                if (skuIdOrCode) {
                    ...
                    if (matched)
                        return { id: matched.id, skuCode: matched.sku_code, name: matched.name, uom: matched.uom || "Bottles" };
                }
                return { id: skuRows[0].id, skuCode: skuRows[0].sku_code, name: skuRows[0].name, uom: skuRows[0].uom || "Bottles" };
            }
            const globalRes = await database_js_1.db.execute((0, drizzle_orm_1.sql) `SELECT id, sku_code, name, uom FROM public.skus LIMIT 1`);
    ...
    mapOrderRow(row, skuMap) {
        ...
        if (upperSt === "OPEN")
            status = "Open";
        else if (upperSt === "ALLOCATED")
            status = "Allocated";
        else if (upperSt === "SCHEDULED")
            status = "Scheduled";
        else if (upperSt === "IN_PRODUCTION" || upperSt === "IN PRODUCTION")
            status = "In Production";
        else if (upperSt === "FULFILLED")
            status = "Fulfilled";
        else if (upperSt === "CANCELLED" || upperSt === "CANCELED")
            status = "Cancelled";
        else if (upperSt === "CONFIRMED")
            status = "Open";
        else
    ...
    async deleteForecast(tenantId, id) {
        ...
        const condition = isUuid
            ? (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(planning_js_1.forecasts.tenantId, tenantId), (0, drizzle_orm_1.eq)(planning_js_1.forecasts.id, id))
            : (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(planning_js_1.forecasts.tenantId, tenantId), (0, drizzle_orm_1.eq)(planning_js_1.forecasts.period, id));
        await database_js_1.db.delete(planning_js_1.forecasts).where(condition);
        return { success: true, id };
    }
    // ============================================================
    ...
    async createDemandHistory(tenantId, input) {
    ...
    async updateDemandHistory(tenantId, id, input) {
    ...
    async deleteDemandHistory(tenantId, id) {
    ...
    async createPromotion(tenantId, input) {
        try {
            const dbRes = await this.createPromotionCampaign(tenantId, "PLT-01", {
                ...
                incrementalUnits: input.incrementalUnits || input.projectedUnits || 5000,
    ...
    async updatePromotionCampaign(tenantId, id, input) {
        if (!(0, tenantContext_js_1.isValidUuid)(id))
            return null;
        const updateValues = { updatedAt: new Date() };
        const promoName = input.name || input.title;
        if (promoName)
            updateValues.name = promoName;
    ...
    mapShipmentRow(row) {
        ...
        let uiStatus = "Booked";
        if (rawStatus.includes("DISPATCH"))
            uiStatus = "Dispatched";
        else if (rawStatus.includes("PENDING"))
            uiStatus = "Pending Dispatch";
        else if (rawStatus.includes("STAGE"))
            uiStatus = "Staged";
    ...
    async listShipments(tenantId, plantId) {
        let rows = await database_js_1.db.select().from(warehouse_js_1.shipmentOrders).where((0, drizzle_orm_1.eq)(warehouse_js_1.shipmentOrders.tenantId, tenantId));
        if (rows.length === 0) {
            // Sync from real customer orders in DB so shipment_orders table is populated with live data
            const currentOrders = await database_js_1.db.select().from(planning_js_1.customerOrders).where((0, drizzle_orm_1.eq)(planning_js_1.customerOrders.tenantId, tenantId));
            if (currentOrders.length > 0) {
                ...
                rows = await database_js_1.db.insert(warehouse_js_1.shipmentOrders).values(initialShipments).returning();
            }
        }
    ...
    async createShipment(tenantId, input) {
        ...
        const [created] = await database_js_1.db
            .insert(warehouse_js_1.shipmentOrders)
            .values({
            tenantId,
            plantId: resolvedPlant,
    ...
    async updateShipmentStatus(tenantId, id, nextStatus) {
        ...
        const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id);
        const condition = isUuid
            ? (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(warehouse_js_1.shipmentOrders.tenantId, tenantId), (0, drizzle_orm_1.eq)(warehouse_js_1.shipmentOrders.id, id))
            : (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(warehouse_js_1.shipmentOrders.tenantId, tenantId), (0, drizzle_orm_1.eq)(warehouse_js_1.shipmentOrders.shipmentNumber, id));
        const [updated] = await database_js_1.db
    ...
    async listApsSchedules(tenantId, plantId) {
        ...
        try {
            const dbSchedules = await database_js_1.db.select().from(planning_js_1.apsSchedules).where((0, drizzle_orm_1.eq)(planning_js_1.apsSchedules.tenantId, validTenant));
            ...
            const inserted = await database_js_1.db.insert(planning_js_1.apsSchedules).values(initialApsSeeds).returning();
    ...
    async resolveLineId(tenantId, lineIdOrCode) {
        const validTenant = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : "5bce8458-909a-4dd2-b221-614c32ac7c89";
        try {
            const allLines = await database_js_1.db.select({ id: masterData_js_1.productionLines.id, name: masterData_js_1.productionLines.name }).from(masterData_js_1.productionLines).where((0, drizzle_orm_1.eq)(masterData_js_1.productionLines.tenantId, validTenant));
            ...
            const globalLines = await database_js_1.db.select({ id: masterData_js_1.productionLines.id }).from(masterData_js_1.productionLines).limit(1);
            if (globalLines.length > 0)
                return globalLines[0].id;
        }
        catch (_) { }
    ...
    async runMrpExplosion(tenantId, plantId) {
        ...
        let rows = await database_js_1.db
            .select({
            id: planning_js_1.mrpRequirements.id,
            tenantId: planning_js_1.mrpRequirements.tenantId,
            plantId: planning_js_1.mrpRequirements.plantId,
            skuId: planning_js_1.mrpRequirements.skuId,
            materialName: planning_js_1.mrpRequirements.materialName,
            skuCode: planning_js_1.mrpRequirements.skuCode,
            category: planning_js_1.mrpRequirements.category,
            uom: planning_js_1.mrpRequirements.uom,
            grossRequirement: planning_js_1.mrpRequirements.grossRequirement,
            safetyStock: planning_js_1.mrpRequirements.safetyStock,
            availableStock: planning_js_1.mrpRequirements.availableStock,
            reservedStock: planning_js_1.mrpRequirements.reservedStock,
            scheduledReceipts: planning_js_1.mrpRequirements.scheduledReceipts,
            netShortage: planning_js_1.mrpRequirements.netShortage,
            requiredDate: planning_js_1.mrpRequirements.requiredDate,
            status: planning_js_1.mrpRequirements.status,
            suggestedAction: planning_js_1.mrpRequirements.suggestedAction,
            calculatedAt: planning_js_1.mrpRequirements.calculatedAt,
            dbSkuName: masterData_js_1.skus.name,
            dbSkuCode: masterData_js_1.skus.skuCode,
            dbCategory: masterData_js_1.skus.category,
            dbUom: masterData_js_1.skus.uom,
        })
            .from(planning_js_1.mrpRequirements)
            .leftJoin(masterData_js_1.skus, (0, drizzle_orm_1.eq)(planning_js_1.mrpRequirements.skuId, masterData_js_1.skus.id))
            .where((0, drizzle_orm_1.eq)(planning_js_1.mrpRequirements.tenantId, tenantId))
            .orderBy((0, drizzle_orm_1.sql) `${planning_js_1.mrpRequirements.calculatedAt} DESC`);
        return rows.map((r) => {
    ...
    async getMrpRequirementById(tenantId, id) {
        const isUuid = (0, tenantContext_js_1.isValidUuid)(id);
        const condition = isUuid
            ? (0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(planning_js_1.mrpRequirements.tenantId, tenantId), (0, drizzle_orm_1.eq)(planning_js_1.mrpRequirements.id, id))
            : (0, drizzle_orm_1.eq)(planning_js_1.mrpRequirements.tenantId, tenantId);
        const [found] = await database_js_1.db.select().from(planning_js_1.mrpRequirements).where(condition).limit(1);
        return found || null;
    }
    async updateMrpRequirement(tenantId, id, input) {
        if (!(0, tenantContext_js_1.isValidUuid)(id))
            return null;
        const updateValues = { calculatedAt: new Date() };
        ...
        const [updated] = await database_js_1.db
            .update(planning_js_1.mrpRequirements)
            .set(updateValues)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(planning_js_1.mrpRequirements.tenantId, tenantId), (0, drizzle_orm_1.eq)(planning_js_1.mrpRequirements.id, id)))
            .returning();
        return updated;
    }
    async deleteMrpRequirement(tenantId, id) {
        const isUuid = (0, tenantContext_js_1.isValidUuid)(id);
        if (isUuid) {
            await database_js_1.db
                .delete(planning_js_1.mrpRequirements)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(planning_js_1.mrpRequirements.tenantId, tenantId), (0, drizzle_orm_1.eq)(planning_js_1.mrpRequirements.id, id)));
        }
        else {
            await database_js_1.db
                .delete(planning_js_1.mrpRequirements)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(planning_js_1.mrpRequirements.tenantId, tenantId), (0, drizzle_orm_1.or)((0, drizzle_orm_1.eq)(planning_js_1.mrpRequirements.skuCode, id), (0, drizzle_orm_1.eq)(planning_js_1.mrpRequirements.skuId, id))));
        }
        return { success: true, id };
    }
    async generateMrpBaselineRequirements(tenantId, plantId) {
        const resolvedPlant = await this.resolvePlantId(tenantId, plantId);
        const allSkus = await database_js_1.db.select().from(masterData_js_1.skus).where((0, drizzle_orm_1.eq)(masterData_js_1.skus.tenantId, tenantId));
        ...
        await database_js_1.db.delete(planning_js_1.mrpRequirements).where((0, drizzle_orm_1.eq)(planning_js_1.mrpRequirements.tenantId, tenantId));
        ...
        if (initialInserts.length > 0) {
            await database_js_1.db.insert(planning_js_1.mrpRequirements).values(initialInserts);
        }
    ...
    async listServiceRisks(tenantId, plantId) {
        try {
            if ((0, tenantContext_js_1.isValidUuid)(tenantId)) {
                const rows = await database_js_1.db.select().from(planning_js_1.serviceRisks).where((0, drizzle_orm_1.eq)(planning_js_1.serviceRisks.tenantId, tenantId));
                ...
                await database_js_1.db.insert(planning_js_1.serviceRisks).values(seeds).onConflictDoNothing();
                const freshRows = await database_js_1.db.select().from(planning_js_1.serviceRisks).where((0, drizzle_orm_1.eq)(planning_js_1.serviceRisks.tenantId, tenantId));
    ...
    async mitigateServiceRisk(tenantId, plantId, input) {
        ...
        try {
            if ((0, tenantContext_js_1.isValidUuid)(tenantId)) {
                await database_js_1.db.update(planning_js_1.serviceRisks)
                    .set({
                    isMitigated: true,
        ...
        const activeCount = currentList.filter((r) => !r.isMitigated).length;
        const mitigatedCount = currentList.filter((r) => r.isMitigated).length;
        const remainingExposure = currentList.filter((r) => !r.isMitigated).reduce((sum, r) => sum + (r.financialExposure || 0), 0);
        return {
            success: true,
            riskId: input.riskId,
    ...
    async getSupplyDemandBalance(tenantId, plantId) {
        const allSkus = await database_js_1.db.select().from(masterData_js_1.skus).where((0, drizzle_orm_1.eq)(masterData_js_1.skus.tenantId, tenantId));
        const orders = await database_js_1.db.select().from(planning_js_1.customerOrders).where((0, drizzle_orm_1.eq)(planning_js_1.customerOrders.tenantId, tenantId));
        const fcs = await database_js_1.db.select().from(planning_js_1.forecasts).where((0, drizzle_orm_1.eq)(planning_js_1.forecasts.tenantId, tenantId));
        const schedules = await database_js_1.db.select().from(planning_js_1.apsSchedules).where((0, drizzle_orm_1.eq)(planning_js_1.apsSchedules.tenantId, tenantId));
        if (allSkus.length === 0) {
            return {
                totalFirmDemand: 0,
    ...
    // ============================================================
    async ensureScheduleVersionsTable() {
        try {
    ...
    async listScheduleVersions(tenantId) {
        ...
        try {
            const dbRows = await database_js_1.db
                .select()
                .from(planning_js_1.pmScheduleVersions)
                .where((0, drizzle_orm_1.eq)(planning_js_1.pmScheduleVersions.tenantId, validTenant))
                .orderBy((0, drizzle_orm_1.sql) `${planning_js_1.pmScheduleVersions.createdAt} DESC`);
            ...
            const insertedRows = await database_js_1.db.insert(planning_js_1.pmScheduleVersions).values(initialSeeds).returning();
    ...
    async createScheduleVersion(tenantId, input) {
        ...
        try {
            const activeDbSchedules = await database_js_1.db.select().from(planning_js_1.apsSchedules).where((0, drizzle_orm_1.eq)(planning_js_1.apsSchedules.tenantId, validTenant));
        ...
        const existingCountResult = await database_js_1.db
        ...
        try {
            const [newVersionRow] = await database_js_1.db.insert(planning_js_1.pmScheduleVersions).values(insertPayload).returning();
            const formatted = {
                id: newVersionRow.id,
            ...
            this.scheduleVersionsStore.unshift(formatted);
            return formatted;
        }
        catch (err) {
    ...
    async getProcessingBatches(tenantId) {
        const client = await database_js_1.pool.connect();
        try {
    ...
    async createProcessingBatch(tenantId, plantId, payload) {
        const client = await database_js_1.pool.connect();
        try {
            let validTenant = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : null;
            if (validTenant) {
            ...
            if (!validTenant) {
                ...
                if (tRes.rows.length > 0)
                    validTenant = tRes.rows[0].id;
                else
                    validTenant = "5bce8458-909a-4dd2-b221-614c32ac7c89";
            }
            let resolvedPlant = (0, tenantContext_js_1.isValidUuid)(plantId) ? plantId : null;
            if (resolvedPlant) {
            ...
            if (!poId || !(0, tenantContext_js_1.isValidUuid)(poId)) {
            ...
            if (!targetSkuId || !(0, tenantContext_js_1.isValidUuid)(targetSkuId)) {
    ...
    async linkBatchToPackagingOrder(batchId, productionOrderId) {
        const client = await database_js_1.pool.connect();
        try {
            await client.query(`
