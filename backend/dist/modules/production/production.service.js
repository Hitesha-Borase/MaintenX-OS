...
class ProductionService {
    async listOrders(tenantId, plantId) {
        try {
            const orders = await database_js_1.db.query.productionOrders.findMany({
    ...
    async getProductionPerformance(plantId) {
        return {
            plantCode: plantId || "PLT-01",
