...
class WarehouseService {
    ...
    async getDashboardStats(tenantId, plantId) {
        let lots = [];
        try {
            lots = await database_js_1.db.select().from(warehouse_js_1.inventoryLots).where((0, tenantContext_js_1.isValidUuid)(tenantId) ? (0, drizzle_orm_1.eq)(warehouse_js_1.inventoryLots.tenantId, tenantId) : (0, drizzle_orm_1.sql) `1=1`);
        }
        catch (err) {
        ...
        try {
            const fgRows = await database_js_1.db.select().from(warehouse_js_1.finishedGoods).where((0, tenantContext_js_1.isValidUuid)(tenantId) ? (0, drizzle_orm_1.eq)(warehouse_js_1.finishedGoods.tenantId, tenantId) : (0, drizzle_orm_1.sql) `1=1`);
        ...
        let ordersCount = 0;
        try {
            const orders = await database_js_1.db.select().from(warehouse_js_1.shipmentOrders).where((0, tenantContext_js_1.isValidUuid)(tenantId) ? (0, drizzle_orm_1.eq)(warehouse_js_1.shipmentOrders.tenantId, tenantId) : (0, drizzle_orm_1.sql) `1=1`);
        ...
        try {
            const rcvRows = await database_js_1.db.select().from(warehouse_js_1.wmsReceiving).where((0, tenantContext_js_1.isValidUuid)(tenantId) ? (0, drizzle_orm_1.eq)(warehouse_js_1.wmsReceiving.tenantId, tenantId) : (0, drizzle_orm_1.sql) `1=1`);
    ...
    async quickReceive(tenantId, plantId, data) {
        ...
        let effectiveTenantId = (0, tenantContext_js_1.isValidUuid)(tenantId) ? tenantId : "0f63be8b-52aa-4e6b-ab83-1d5477328262";
        let effectivePlantId = ((0, tenantContext_js_1.isValidUuid)(plantId) && plantId) ? plantId : "";
        if (!effectivePlantId) {
            const pRows = await database_js_1.db.select().from(tenants_js_1.plants).where((0, drizzle_orm_1.eq)(tenants_js_1.plants.tenantId, effectiveTenantId)).limit(1);
            effectivePlantId = pRows[0]?.id || "6869789b-32d4-4911-bf29-74a9e338f14a";
        }
        try {
            // 1. Find or create matching SKU
            let [matchedSku] = await database_js_1.db
                .select()
                .from(masterData_js_1.skus)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(masterData_js_1.skus.tenantId, effectiveTenantId), (0, drizzle_orm_1.or)((0, drizzle_orm_1.eq)(masterData_js_1.skus.skuCode, newLot.materialCode), (0, drizzle_orm_1.eq)(masterData_js_1.skus.name, newLot.materialName))))
                .limit(1);
            if (!matchedSku) {
                [matchedSku] = await database_js_1.db
            ...
            const [insertedLot] = await database_js_1.db
                .insert(warehouse_js_1.inventoryLots)
                .values({
                tenantId: effectiveTenantId,
                plantId: effectivePlantId,
