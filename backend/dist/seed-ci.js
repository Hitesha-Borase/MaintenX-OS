...
async function seedCI() {
    ...
    `);
      ...
      (
        ...
        '["Ronald Robinson (QA Manager)", "David Markov (Maintenance Lead)", "Carlos Gomez (Tech)"]'::jsonb,
        '2026-08-28',
        3,
        '[
        ...
        '{
          "d1Team": "David Kim (Lead), David Markov (Maint), Ronald Robinson (QA)",
      ...
      (
        ...
        'Ronald Robinson (QA Manager)',
        '["David Kim (Lead CI)", "David Markov (Maintenance Lead)"]'::jsonb,
        '2026-08-25',
        6,
        '[
        ...
        '{
          "d1Team": "Ronald Robinson (QA Lead), David Markov (Maint)",
    ...
    `);
      ...
      VALUES
      ('EVD-01', 'RCA-2026-001', 'SCADA Trend', 'Steam Manifold Pressure & Pasteurizer Temp Plunge Log', 'Recorded pressure plunge from 6.2 bar to 2.8 bar at 10:22 AM on Line 2.', '', 'David Markov', '2026-08-28'),
      ...
      ('EVD-03', 'RCA-2026-002', 'Lab Torque Curve', 'Cap Torque Failure Frequency Distribution', '98% of out-of-spec caps isolated specifically to Spindle Station #4.', '', 'Ronald Robinson', '2026-08-25')
      ON CONFLICT (id) DO NOTHING;
    `);
        // 4. Hypotheses
        await client.query(`
      ...
      ('HYP-02', 'RCA-2026-001', 'Boiler feed steam boiler water treatment scale caused valve seat binding.', 'Inspect valve stem and seat with borescope.', 'Valve seat was pristine with zero scaling or mechanical scoring.', 'Refuted', 'David Markov', '2026-08-29 11:30'),
      ('HYP-03', 'RCA-2026-002', 'Spindle 4 magnetic clutch set-screw loosened due to lack of threadlocker.', 'Check fastener torque on all 24 spindles using calibrated digital wrench.', 'Spindle 4 fastener was loose at 0.4 Nm (spec: 4.5 Nm).', 'Confirmed Root Cause', 'Ronald Robinson', '2026-08-26 16:20')
      ON CONFLICT (id) DO NOTHING;
    `);
        // 5. CAPA Actions
        await client.query(`
      ...
      VALUES
      ('CAPA-2026-001', 'RCA-2026-001', 'PRJ-CI-001', 'Install redundant 0.01 micron coalescing filter on instrument air supply skid.', 'Corrective', 'David Markov (Maintenance Lead)', '2026-09-10', 'High', 'In Progress', null, 'Procurement PO-SUP-401 placed; technician scheduled for installation.', null, null, null),
      ...
      ('CAPA-2026-003', 'RCA-2026-002', 'PRJ-CI-002', 'Standardize Loctite 243 threadlocker on all 24 filler capping spindle rebuild procedures.', 'Corrective', 'David Markov (Maintenance Lead)', '2026-08-28', 'High', 'Verified', '2026-08-27', 'All 24 spindles re-torqued and verified with Loctite 243.', 'Zero cap torque deviations observed across 180,000 bottles.', 'Ronald Robinson (QA Manager)', '2026-08-29 10:00'),
      ('CAPA-2026-004', 'RCA-2026-002', 'PRJ-CI-002', 'Establish automated torque inspection check in hourly Quality Pitch record.', 'Preventive', 'Ronald Robinson (QA Manager)', '2026-08-20', 'Medium', 'In Progress', null, 'Electronic quality sheet draft prepared.', null, null, null)
      ON CONFLICT (id) DO NOTHING;
    `);
        // 6. CI Projects
        await client.query(`
      ...
      ('PRJ-CI-002', 'CIP Cycle Time & Water Consumption Reduction', 'Kaizen Event', 'PLT-01', 'LIN-01', 'AST-001', 'RCA-2026-002', 'Sustainability & Operations Lead', 'David Markov (Maintenance Lead)', '2026-07-15', '2026-08-31', 'Completed', 100, '65 min CIP Cycle / 14,000 L Water', '45 min CIP Cycle / 9,500 L Water', '42 min CIP Cycle / 9,100 L Water', 18000, 18000, 'Verified & Locked', 'Stephanie Kuzmych (Plant Director)', '2026-08-31 16:45'),
      ('PRJ-CI-003', 'Label Application Defect Elimination & Vision Upgrade', 'SMED Rapid Setup', 'PLT-01', 'LIN-01', 'AST-004', null, 'Packaging Department Head', 'Ronald Robinson (QA Manager)', '2026-08-10', '2026-09-15', 'In Progress', 65, '1.8% Label Skew Defect Rate', '< 0.2% Defect Rate', '0.4% Defect Rate', 11200, 8400, 'Draft', null, null)
      ON CONFLICT (id) DO NOTHING;
    `);
        // 7. Losses
        await client.query(`
    ...
    `);
      ...
      VALUES
      ('STD-ENG-001', 'SOP-ENG-402: Instrument Air Header Filtration & Dewpoint Monitoring', 'Controlled SOP', 'v2.1', 'PLT-01', 'LIN-02', 'AST-002', 'PRJ-CI-001', 'RCA-2026-001', 'Engineering Quality Committee', 'Active', '2026-08-15', '2027-08-15', 'Stephanie Kuzmych (Plant Director)'),
      ('STD-ENG-002', 'SOP-CAP-002: Rotary Capper Spindle Rebuild & Loctite 243 Fastener Standard', 'Engineering Spec', 'v1.4', 'PLT-01', 'LIN-01', 'AST-001', 'PRJ-CI-002', 'RCA-2026-002', 'David Markov (Maintenance Lead)', 'Active', '2026-08-28', '2027-08-28', 'Stephanie Kuzmych (Plant Director)'),
      ('STD-ENG-003', 'HACCP-CCP-01: Thermal Pasteurization Continuous Flow Critical Limit Standard', 'HACCP Limit', 'v3.0', 'PLT-01', 'LIN-02', 'AST-002', 'PRJ-CI-001', 'RCA-2026-001', 'Ronald Robinson (QA Manager)', 'Active', '2026-08-01', '2027-08-01', 'QA Governance Board')
      ON CONFLICT (id) DO NOTHING;
    `);
