...
export class DashboardsService {
    ...
    try {
      ...
      if (targetOrder) {
        ...
        const userRes = await db.execute(sql`
          ...
          LIMIT 1
        `);
        const uRows = (userRes as any)?.rows || (Array.isArray(userRes) ? userRes : []);
        if (uRows.length > 0 && uRows[0].id) {
          operatorId = uRows[0].id;
        } else {
          const fallbackUser = await db.execute(sql`
            SELECT id FROM public.users WHERE tenant_id = ${tenantIdForLog} LIMIT 1
          `);
          const fbRows = (fallbackUser as any)?.rows || (Array.isArray(fallbackUser) ? fallbackUser : []);
          if (fbRows.length > 0 && fbRows[0].id) operatorId = fbRows[0].id;
        }
  ...
  async logScrapDefect(tenantId: string, payload: { defectCode: string; scrapAdd: number; notes?: string; orderNumber?: string }) {
    ...
    try {
      ...
      if (uRows.length > 0 && uRows[0].id) userId = uRows[0].id;
      ...
      if (pRows.length > 0 && pRows[0].id) plantId = pRows[0].id;
  ...
  async getSupervisorStaffing(tenantId: string) {
    try {
      ...
      const today = new Date().toISOString().substring(0, 10);
      const rosters = shiftsRes.rows.map((sh: any) => ({
        id: sh.id,
        shiftCode: sh.code,
        shiftName: sh.name,
  ...
  async deleteSupervisorStaffing(tenantId: string, id: string) {
    const { validTenant } = await resolveValidTenantAndPlant(tenantId);
    try {
      await pool.query(`DELETE FROM public.shifts WHERE id = $1 AND tenant_id = $2;`, [id, validTenant]);
      return {
        id,
  ...
  async getSupervisorProfile(tenantId: string, userId?: string) {
    try {
      ...
      const matchingStaff = await db
        .select()
        .from(staff)
        .where(and(eq(staff.tenantId, validTenant), eq(staff.name, userName)));
