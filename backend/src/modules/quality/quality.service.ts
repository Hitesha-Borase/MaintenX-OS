...
import { finishedGoods } from "../../db/schema/warehouse.js";
import { isValidUuid } from "../../shared/utils/tenantContext.js";

let inMemoryAllergenAudits: any[] = [];
let inMemoryLineReadiness: any[] = [];
let inMemoryCleaningVerification: any = {
  verified: false,
  atpTestResult: "-",
  microbialResidue: "-",
  targetLimit: "-",
...
let inMemoryProcessChecks: any[] = [];
let inMemoryProductChecks: any[] = [];
...
export class QualityService {
  ...
  async getQualitySummary(tenantId: string) {
    ...
    try {
      ...
      if (preOpRes.rows[0]) {
        ...
        if (st === "READY" || st === "PASSED") line1PreOp = "PASSED";
        else if (st === "FAILED" || st === "FAIL") line1PreOp = "FAILED";
        else line1PreOp = st || "NOT STARTED";
      } else {
  ...
  async getQaReleaseMetrics(tenantId: string) {
    ...
    const pendingQueue = await db
      ...
      .where(
        isValidUuid(tenantId)
          ? and(
              eq(qaReleaseQueue.tenantId, tenantId),
    ...
    const allCcp = await db
      .select()
      .from(ccpChecks)
      .where(isValidUuid(tenantId) ? eq(ccpChecks.tenantId, tenantId) : undefined);
    ...
    const approvedReleasesList = await db
      ...
      .where(isValidUuid(tenantId) ? eq(qaApprovedReleases.tenantId, tenantId) : undefined);
  ...
  async getBatchReleaseDossier(tenantId: string, batchId: string) {
    const [queueItem] = await db
      ...
      .where(
        and(
          isValidUuid(tenantId) ? eq(qaReleaseQueue.tenantId, tenantId) : undefined,
    ...
    const [reviewItem] = await db
      ...
      .where(
        and(
          isValidUuid(tenantId) ? eq(batchQualityReviews.tenantId, tenantId) : undefined,
  ...
  async authorizeBatchRelease(tenantId: string, plantId: string, input: any, userId: string, ipAddress?: string) {
    ...
    if (input.batchId && isValidUuid(input.batchId)) {
      ...
      const [found] = await db.select().from(batches).where(and(eq(batches.tenantId, tenantId), eq(batches.batchNumber, input.batchId))).limit(1);
      batch = found;
    }
    ...
    if (!batch) {
      throw new NotFoundError("No active batch found for release authorization");
    }
    ...
    try {
      const [inserted] = await db
        ...
        .values({
          ...
          dispositionBy: userId && isValidUuid(userId) ? userId : null,
    ...
    if (input.disposition === "RELEASED") {
      ...
      try {
        ...
        if (!existingHist) {
          await db.insert(batchHistory).values({
            ...
            recipe: (batch as any).sku?.name || (batch as any).recipeName || "Finished Goods SKU",
      ...
      try {
        ...
        await db.insert(qaApprovedReleases).values({
          ...
          releaseCode,
          batchId: batch.batchNumber,
          recipe: (batch as any).sku?.name || (batch as any).recipeName || (batch as any).productName || "Finished Goods SKU",
          pallets: "Standard Lot",
          approvedBy: "",
      ...
      try {
        const [existingFg] = await db
          .select()
          .from(finishedGoods)
          .where(eq(finishedGoods.batchNumber, batch.batchNumber))
          .limit(1);
        ...
        if (!existingFg) {
          ...
          await (db.insert(finishedGoods) as any).values({
            tenantId: isValidUuid(tenantId) ? tenantId : "0f63be8b-52aa-4e6b-ab83-1d5477328262",
            plantId: isValidUuid(plantId) ? plantId : "6869789b-32d4-4911-bf29-74a9e338f14a",
            sku: (batch as any).sku?.skuCode || "SKU-004",
            productName: (batch as any).sku?.name || (batch as any).recipeName || "SD HD",
  ...
  async listQualityHolds(tenantId: string) {
    const records = await db
      .select()
      .from(qualityHolds)
      .where(isValidUuid(tenantId) ? eq(qualityHolds.tenantId, tenantId) : undefined)
  ...
  async listDeviations(tenantId: string) {
    const records = await db
      .select()
      .from(deviations)
      .where(isValidUuid(tenantId) ? eq(deviations.tenantId, tenantId) : undefined)
  ...
  async listInvestigations(tenantId: string) {
    const records = await db
      ...
      .where(isValidUuid(tenantId) ? eq(qualityInvestigations.tenantId, tenantId) : undefined)
  ...
  async listNcrReports(tenantId: string) {
    const records = await db
      .select()
      .from(ncrs)
      .where(isValidUuid(tenantId) ? eq(ncrs.tenantId, tenantId) : undefined)
  ...
  async listLineReadiness(tenantId: string) {
    ...
    try {
      ...
      const linesRes = await client.query(
        `SELECT id, code, name, nominal_speed_bpm FROM public.production_lines WHERE tenant_id = $1 ORDER BY code ASC;`,
        [tenantId]
      );
  ...
  async toggleLineReadiness(tenantId: string, plantId: string, input: any, userId: string) {
    ...
    try {
      ...
      const check = await client.query(
        `SELECT id FROM public.line_readiness WHERE (id::text = $1 OR line_code = $2 OR line = $3) AND tenant_id = $4;`,
        [String(input.lineId || ""), input.lineCode || "", input.lineName || "", tenantId]
      );
      ...
      if (check.rows.length > 0) {
        await client.query(
          `UPDATE public.line_readiness 
           ...
           WHERE id = $6;`,
          [newStatus, safety, sanitation, mechanical, lastInspection, check.rows[0].id]
        );
      } else {
        await client.query(
          `INSERT INTO public.line_readiness 
           (tenant_id, line, line_code, status, safety, sanitation, mechanical, speed_target, last_inspection)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9);`,
          [tenantId, input.lineName || "Line", input.lineCode || "LIN-01", newStatus, safety, sanitation, mechanical, input.speedTarget || "500 BPM", lastInspection]
        );
      }
  ...
  async verifyCleaning(tenantId: string, plantId: string, input: any, userId: string) {
    ...
    try {
      ...
      const existing = await client.query(
        `SELECT id FROM public.cleaning_verifications WHERE tenant_id = $1 ORDER BY id DESC LIMIT 1;`,
        [tenantId]
      );
      ...
      if (existing.rows.length > 0) {
        await client.query(
          `UPDATE public.cleaning_verifications 
           ...
           WHERE id = $6;`,
          [notes, verifiedBy, now, atpResult, loop, existing.rows[0].id]
        );
      } else {
        await client.query(
          `INSERT INTO public.cleaning_verifications 
           (tenant_id, verified, status, notes, verified_by, verified_at, atp_test_result, loop)
           VALUES ($1, TRUE, 'VERIFIED', $2, $3, $4, $5, $6);`,
          [tenantId, notes, verifiedBy, now, atpResult, loop]
        );
      }
    ...
    try {
      await client.query(
        `UPDATE public.cleaning_verifications 
         ...
         WHERE id = (SELECT id FROM public.cleaning_verifications WHERE tenant_id = $1 ORDER BY id DESC LIMIT 1);`,
        [tenantId]
      );
  ...
  async listDispositionRelease(tenantId: string) {
    const records = await db
      ...
      .where(
        and(
          isValidUuid(tenantId) ? eq(qualityHolds.tenantId, tenantId) : undefined,
  ...
  async listCapaRecords(tenantId: string) {
    const rows = await db
      ...
      .from(capaRecords)
      .where(
        isValidUuid(tenantId) ? eq(capaRecords.tenantId, tenantId) : undefined
      )
  ...
  async listAuditTrail(tenantId: string) {
    const rows = await db
      ...
      .from(qaAuditTrail)
      .where(
        isValidUuid(tenantId) ? eq(qaAuditTrail.tenantId, tenantId) : undefined
      )
  ...
  async listQualityReports(tenantId: string) {
    const rows = await db
      ...
      .from(qaReports)
      .where(
        isValidUuid(tenantId) ? eq(qaReports.tenantId, tenantId) : undefined
      )
  ...
  async listNotifications(tenantId: string) {
    const rows = await db
      ...
      .where(
        isValidUuid(tenantId) ? eq(qaNotifications.tenantId, tenantId) : undefined
      )
  ...
  async updateQualityProfile(tenantId: string, input: any, userId: string) {
    if (input.signaturePin) {
      await db
        ...
        .where(
          isValidUuid(tenantId) ? eq(qaProfiles.tenantId, tenantId) : undefined
        );
    }
  ...
  async getSanitationChecklist(tenantId: string) {
    try {
      ...
      const [config] = await db
        ...
        .where(eq(sanitationCipConfig.tenantId, tenantId))
        .limit(1);
  ...
  async listQualityRecords(tenantId: string) {
    const records = await db
      ...
      .where(isValidUuid(tenantId) ? eq(batchQualityRecords.tenantId, tenantId) : undefined)
