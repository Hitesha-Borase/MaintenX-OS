...
export function PreventiveActions() {
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `Preventive_Actions_CAPA_${new Date().toISOString().substring(0, 10)}.csv`;
    a.click();
  ...
  const filteredActions = useMemo(() => {
    ...
    return preventiveList.filter((a) => {
  ...
  }, [preventiveList, statusFilter, priorityFilter, searchQuery]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px", width: "100%", maxWidth: "1600px", margin: "0 auto", minWidth: 0 }}>
        ...
        <div style={{ minWidth: "240px", flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
            <h1 style={{ fontSize: "clamp(18px, 4vw, 24px)", fontWeight: 800, color: "var(--text-primary)", letterSpacing: "-0.3px", lineHeight: 1.2 }}>
              CAPA — Preventive Actions & Poka-Yoke Controls
            </h1>
            <Badge variant="cyan">{preventiveList.length} PREVENTIVE ACTIONS</Badge>
          </div>
        </div>
      ...
      >
        <StatCard
          title="Open Preventive Controls"
          value={preventiveList.filter((a) => a.status === "Open" || a.status === "In Progress").length.toString()}
          unit="In Implementation"
          icon={Clock}
          colorVariant="cyan"
        />
        <StatCard
          title="Poka-Yoke Safeguards"
          value="100%"
          unit="System Safeguard"
          icon={ShieldCheck}
          colorVariant="emerald"
        />
        <StatCard
          title="Implemented & Verified"
          value={preventiveList.filter((a) => a.status === "Completed" || a.status === "Verified" || a.status === "Closed").length.toString()}
          unit="Active Controls"
          icon={CheckCircle2}
          colorVariant="emerald"
        />
        <StatCard
      ...
      >
        ...
        >
          <div style={{ position: "relative", minWidth: "240px", flex: 1 }}>
            ...
            <input
              type="text"
              placeholder="Search preventive action, ID, owner or RCA ID..."
              value={searchQuery}
          ...
          <table className="data-table" style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
            <thead>
              <tr style={{ borderBottom: "1px solid var(--border-subtle)", backgroundColor: "var(--bg-card-subtle)" }}>
                <th style={{ padding: "12px 16px", fontSize: "11px", fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase" }}>Preventive Action Scope</th>
                <th style={{ padding: "12px 16px", fontSize: "11px", fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase" }}>Linked RCA</th>
                <th style={{ padding: "12px 16px", fontSize: "11px", fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase" }}>Governance Owner</th>
                    ...
                    <td style={{ padding: "12px 16px", textAlign: "right" }}>
                      <div style={{ display: "inline-flex", alignItems: "center", gap: "6px" }}>
                        ...
                        {a.status === "In Progress" && (
                          <button
                            onClick={() => updateCapaStatus(a.id, "Completed", "Preventive standard published")}
                        ...
                        {a.status === "Completed" && (
                          <button
                            onClick={() => updateCapaStatus(a.id, "Verified", "Poka-Yoke effectiveness verified")}
        ...
        <div className="modal-backdrop" onClick={() => setIsModalOpen(false)}>
          <div className="modal-content" style={{ maxWidth: "520px", margin: "16px" }} onClick={(e) => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px", borderBottom: "1px solid var(--border-subtle)", backgroundColor: "var(--bg-card-subtle)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                ...
                <h2 style={{ fontSize: "16px", fontWeight: 800, color: "var(--text-primary)", margin: 0 }}>
                  Create Preventive Action / Poka-Yoke
                </h2>
              </div>
            ...
            <form onSubmit={handleAdd} style={{ padding: "20px", display: "flex", flexDirection: "column", gap: "14px" }}>
              ...
              <div>
                <label className="form-label">Preventive Action Description *</label>
                <textarea
                  rows={3}
                  required
                  placeholder="Describe the system standard, PM checklist change, or automated Poka-Yoke safeguard..."
